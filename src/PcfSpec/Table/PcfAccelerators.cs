...
public class PcfAccelerators : IPcfTable
{
    ...
    public static bool Equals(PcfAccelerators? objA, PcfAccelerators? objB)
    {
        ...
        if ((objA.CompatInfo is null && objB.CompatInfo is not null) || (objA.CompatInfo is not null && objB.CompatInfo is null))
        {
            return false;
        }
        if (objA.CompatInfo is not null && objB.CompatInfo is not null)
        {
            var (rawChunkA, tableSizeA) = objA.CompatInfo.Value;
            var (rawChunkB, tableSizeB) = objB.CompatInfo.Value;
            if (!rawChunkA.SequenceEqual(rawChunkB) || tableSizeA != tableSizeB)
            {
                return false;
            }
        }
        return Equals(objA.TableFormat, objB.TableFormat) &&
