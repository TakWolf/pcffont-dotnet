...
public class PcfFont : IDictionary<PcfTableType, IPcfTable>
{
    ...
    private static void CheckTableType(PcfTableType tableType, IPcfTable table)
    {
        ...
        {
            throw new ArgumentException("Table type mismatch.");
        }
    }
    ...
    private readonly SortedDictionary<PcfTableType, IPcfTable> _tables = new();

    public int Count => _tables.Count;

    bool ICollection<KeyValuePair<PcfTableType, IPcfTable>>.IsReadOnly => false;

    public ICollection<PcfTableType> Keys => _tables.Keys;

    public ICollection<IPcfTable> Values => _tables.Values;

    public IEnumerator<KeyValuePair<PcfTableType, IPcfTable>> GetEnumerator() => _tables.GetEnumerator();
    ...
    public bool TryGetValue(PcfTableType key, [MaybeNullWhen(false)] out IPcfTable value) => _tables.TryGetValue(key, out value);

    public bool ContainsKey(PcfTableType key) => _tables.ContainsKey(key);

    public bool ContainsValue(IPcfTable value) => _tables.ContainsValue(value);

    bool ICollection<KeyValuePair<PcfTableType, IPcfTable>>.Contains(KeyValuePair<PcfTableType, IPcfTable> item) => (_tables as ICollection<KeyValuePair<PcfTableType, IPcfTable>>).Contains(item);
    ...
    void ICollection<KeyValuePair<PcfTableType, IPcfTable>>.Add(KeyValuePair<PcfTableType, IPcfTable> item)
    {
        ...
        (_tables as ICollection<KeyValuePair<PcfTableType, IPcfTable>>).Add(item);
    }

    public bool Remove(PcfTableType key) => _tables.Remove(key);

    bool ICollection<KeyValuePair<PcfTableType, IPcfTable>>.Remove(KeyValuePair<PcfTableType, IPcfTable> item) => (_tables as ICollection<KeyValuePair<PcfTableType, IPcfTable>>).Remove(item);

    public void Clear() => _tables.Clear();

    void ICollection<KeyValuePair<PcfTableType, IPcfTable>>.CopyTo(KeyValuePair<PcfTableType, IPcfTable>[] array, int arrayIndex) => (_tables as ICollection<KeyValuePair<PcfTableType, IPcfTable>>).CopyTo(array, arrayIndex);
